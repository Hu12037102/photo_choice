...
| Камера | Опциональная плитка камеры в первой ячейке (сохраняет в системную галерею) |
...
| `showCamera` | `Boolean` | `true` | Показать плитку камеры в первой ячейке |
...
├── sample/
├── PRD.md                     # Внутренняя продуктовая спецификация
